...
function ShuffleButton() {
  ...
  const onButtonClick = () => {
    ...
    animate([
      ...sparklesReset,
      [".letter", { y: -32 }, { duration: 0.20, delay: stagger(0.06) }],
  ...
  return (
    <div ref={scope} className="shuffle-button-container">
      <button
        onClick={onButtonClick}
        className="relative rounded-full border-2 border-white px-6 py-2 text-2xl text-black transition-colors hover:bg-blue-100 font-mono"
      >
        <span className="sr-only">Shuffle</span>
        <span className="block h-8 overflow-hidden" aria-hidden>
          {["S", "h", "u", "f", "f", "l", "e"].map((letter, index) => (
            <span
              data-letter={letter}
